...
router.get('/users', managerAuth, async (req, res, next) => {
  ...
  User.find(op).skip(page * size).limit(size).exec((err, docs) => {
    if (err) {
      log(err);
...
router.post('/', managerAuth, async (req, res, next) => {
  ...
  if (!user) next(new HttpErrorHandler(400, 'need to specify the user in the request'));
...
router.delete('/project/:projectId/user/:userId', managerAuth, async (req, res, next) => {
  const {
    projectId,
    userId,
  } = req.params;
